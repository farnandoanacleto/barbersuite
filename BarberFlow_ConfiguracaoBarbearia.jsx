...
const css = `
  ...
  /* Layout */
  .cfg-layout { display: flex; min-height: 100vh; }
  .cfg-sidebar {
  ...
  .cfg-content { flex: 1; overflow-y: auto; padding: 22px 24px; }
  ...
  .modal { background: var(--white); border-radius: 14px; padding: 24px; width: 460px; max-width: 100%; animation: mIn 0.18s ease; }
...
function ModalPlano({ plano, onSave, onClose }) {
  ...
  const [form, setForm] = useState(plano || { nome:'', preco:0, visitas:4, ilimitado:false, servicos:[], destaque:false, ativo:true });
  const set = (k,v) => setForm(f=>({...f,[k]:v}));
...
function TabServicos() {
  ...
  const excluir = (id) => setServicos(s=>s.filter(x=>x.id!==id));
  const toggleAtivo = (id) => setServicos(s=>s.map(x=>x.id===id?{...x,ativo:!x.ativo}:x));

  const lista = filtro==='todos' ? servicos : servicos.filter(s=>s.categoria===filtro);

  return (
    <>
      {salvo && <div className="alert-box alert-ok">✓ Serviço salvo com sucesso! O app do cliente já reflete as alterações.</div>}

      <div className="row-between mb-16">
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          {['todos',...CATEGORIAS].map(c=>(
            <button key={c} className={`btn ${filtro===c?'btn-primary':'btn-outline'}`} style={{padding:'5px 12px',fontSize:11}} onClick={()=>setFiltro(c)}>
              {c==='todos'?'Todos':catLabel(c)}
            </button>
          ))}
        </div>
        <button className="btn btn-primary" onClick={()=>setModal({})}>+ Novo serviço</button>
      </div>
      ...
      <div className="tbl-wrap">
        <table>
          <thead>
            <tr>
              <th>Serviço</th><th>Categoria</th><th>Duração</th><th>Preço</th><th>Status</th><th>Ações</th>
            </tr>
          </thead>
          <tbody>
            {lista.map(s=>(
              <tr key={s.id}>
                ...
                <td><span className={catClass(s.categoria)}>{catLabel(s.categoria)}</span></td>
                <td className="text-muted">{s.duracao} min</td>
                <td className="bold text-gold">{R(s.preco)}</td>
                <td>
                  <div className="toggle-wrap" onClick={()=>toggleAtivo(s.id)}>
                    <div className={`toggle ${s.ativo?'on':''}`} style={{width:28,height:16}}/>
                    <span className="text-small text-muted">{s.ativo?'Ativo':'Inativo'}</span>
                  </div>
                </td>
                <td>
                  <div className="btn-row">
                    <button className="btn btn-outline" style={{padding:'4px 10px',fontSize:11}} onClick={()=>setModal(s)}>Editar</button>
                    <button className="btn btn-danger" style={{padding:'4px 10px',fontSize:11}} onClick={()=>excluir(s.id)}>Excluir</button>
                  </div>
                </td>
              </tr>
            ))}
            {lista.length===0 && (
              <tr><td colSpan={6} style={{textAlign:'center',color:'var(--muted)',padding:24}}>Nenhum serviço nesta categoria</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="alert-box alert-info">
        ℹ Os serviços ativos aparecem automaticamente no app do cliente durante o agendamento. Serviços inativos ficam ocultos mas não são excluídos.
      </div>

      {modal !== null && <ModalServico servico={modal} onSave={salvar} onClose={()=>setModal(null)} />}
    </>
  );
}
...
function TabPlanos() {
  ...
  const salvar = (form) => {
    if (form.id) {
      setPlanos(p=>p.map(x=>x.id===form.id?form:x));
    } else {
      setPlanos(p=>[...p,{...form,id:Date.now()}]);
    }
    setModal(null);
    setSalvo(true);
    setTimeout(()=>setSalvo(false),2500);
...
const ABAS = [
  ...
  { key:'integ',    icon:'🔗', label:'Integrações',     section:'Sistema', disabled:true },
  { key:'billing',  icon:'💳', label:'Assinatura SaaS', section:null, disabled:true },
];
